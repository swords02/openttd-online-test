...
template <> SQInteger PushClassName<ScriptError, ScriptType::AI>(HSQUIRRELVM vm) { sq_pushstring(vm, "AIError"); return 1; }

void SQAIError_Register(Squirrel &engine)
{
	DefSQClass<ScriptError, ScriptType::AI> SQAIError("AIError");
	SQAIError.PreRegister(engine, "AIObject");
	...
	SQAIError.DefSQConst(engine, ScriptError::ERR_BRIDGE_TOO_LOW,               "ERR_BRIDGE_TOO_LOW");
	...
	SQAIError.DefSQStaticMethod(engine, &ScriptError::GetErrorCategory,   "GetErrorCategory",   ".");
	SQAIError.DefSQStaticMethod(engine, &ScriptError::GetLastError,       "GetLastError",       ".");
	SQAIError.DefSQStaticMethod(engine, &ScriptError::GetLastErrorString, "GetLastErrorString", ".");
